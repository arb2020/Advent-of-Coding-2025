    ...
    unique case (current_state)
      ...
      EXEC_S: begin
        ...
        if (dir_q == 1'b0) begin
          ...
          if((dial_num != 7'd0) && (step >= dial_num))
            extra = 1'b1;
          else
            extra = 1'b0;
          ...
          if((dial_num != 7'd0) && (step >= (7'd100 - dial_num)))
            extra = 1'b1;
          else
            extra = 1'b0;
        end

        // Calculate modulo_count
        modulo_count = logcnt + extra;
        next_state = COUNT_S;
      end

      COUNT_S: begin
        // Count after the move
        count_next = count + modulo_count;
        next_state = IDLE_S;
      end
